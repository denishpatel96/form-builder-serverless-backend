...
export const handler: APIGatewayProxyHandler = async (event) => {
  ...
  try {
    const { orgId, email } = event.pathParameters;
    ...
    if (!(orgId && email)) {
      return {
        statusCode: 400,
        ...corsHeaders,
        body: JSON.stringify({ message: "orgId and email required" }),
      };
    }
    ...
    if (claimedUsername !== orgId) {
      ...
      if (!(userData && userData.role === "Admin")) {
        return {
          ...
          ...corsHeaders,
          body: JSON.stringify({
            message: "You are not authorized to delete the invitation.",
          }),
        };
      }
    }
    ...
    console.log("Removing org member invitation...");
    await db.send(new DeleteItemCommand(deleteParams));
    return {
      statusCode: 200,
      ...corsHeaders,
      body: JSON.stringify({ message: "Org member invitation removed successfully!" }),
    };
  } catch (e) {
    console.error(e);
    return {
